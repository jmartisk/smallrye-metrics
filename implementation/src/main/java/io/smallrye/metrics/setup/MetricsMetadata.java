...
public class MetricsMetadata {
    ...
    public static List<MetricID> registerMetrics(MetricRegistry registry, MetricResolver resolver, BeanInfo bean,
        ...
        if (counted.isPresent()) {
            ...
            if (registry instanceof MpMetricRegistryAdapter) {
                MetricID metricID = new MetricID(metadata.getName(), tags);
                metricIDs.add(metricID);
                ((MpMetricRegistryAdapter) registry).getMemberToMetricMappings().addMetric(element, metricID,
                        MetricType.COUNTER);
            }
        }
        ...
        if (timed.isPresent()) {
            ...
            if (registry instanceof MpMetricRegistryAdapter) {
                MetricID metricID = new MetricID(metadata.getName(), tags);
                metricIDs.add(metricID);
                ((MpMetricRegistryAdapter) registry).getMemberToMetricMappings().addMetric(element, metricID, MetricType.TIMER);
            }
        }
